...
class GridView: UIView {
    ...
    {
        guard let scrollview = hostScrollView else { return }
        let xOffset = (arbitraryLargeOffset - ((scrollview.frame.size.width - self.frame.size.width) * 0.5)) * 0.5
        let yOffset = (arbitraryLargeOffset - ((scrollview.frame.size.height - self.frame.size.height) * 0.5)) * 0.5
